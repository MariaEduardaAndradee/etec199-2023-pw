...
<body>
        ...
        function redirectDescricao(id) {
            window.location.href = 'http://localhost/SESSAO/descricao.php?id=' + id;
        }
        </script>
...
<?php
        ...
        if ($token != '#bb412abc') {
            // Se a validação estiver incorreta, redirecionar para página de erros
        ...
        $produto_id = $_GET['id'];
        $produtos = json_decode(file_get_contents('http://localhost/JSON/JSON/lista.json'));
